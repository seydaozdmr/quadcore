  ...
  return (
    <div className="space-y-1.5">
      <div className="flex items-center justify-between">
        ...
        <span className={`text-2xl font-bold tabular-nums ${cfg.color}`}>{score}<span className="text-sm font-normal text-slate-500">/100</span></span>
      </div>
...
export default function SmartActionModal({ open, onClose, agendaCardId = '' }: SmartActionModalProps) {
  ...
  function handleSave(actionText: string) {
    addAction({ action: actionText, agendaCardId })
    handleClose()
  }
      ...
      <Dialog.Portal>
        ...
        <Dialog.Content className="fixed left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 z-50 w-full max-w-lg">
          <div className="bg-slate-900 border border-slate-700 rounded-2xl shadow-2xl p-6 space-y-5">
            ...
            {/* Result */}
            {result && cfg && (
              <div className={`border rounded-xl p-4 space-y-4 ${cfg.border}`}>
                {/* Score */}
                ...
                <div className="space-y-1.5">
                  ...
                  <p className="text-sm text-slate-100 bg-slate-800 rounded-lg px-3 py-2.5 leading-relaxed">
